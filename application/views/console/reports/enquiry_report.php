...
<input type="hidden" name="userwallet" id="userwallet" value="<?= ($wallet->balance) ?$wallet->balance :0; ?>">
 <h6 class="primary-title1 mb-0" style="    color: #575757;">Select Filter </h6><br>
          ...
          <form class="cmn-blk mb-4 detail_search" @submit.prevent="getFormValuesSimple">
  ...
  <div class="table_wrapper">
        ...
        <table class="table table-bordered text-nowrap border-bottom enquiry_reports_tbl" id="responsive-datatable-new">
  <thead>
     <tr>
...
<tr v-for="template in dataFromServer" :key="template.id">
                        <td>
                            <label class="custom-control custom-checkbox-md">
                                <input type="checkbox" class="custom-control-input" :value="template.id">
                                <span class="custom-control-label"></span>
                            </label>
                        </td>
                        <td>
                            <div class="cell_content">
                                <p>
                                    {{ template.name }}
                                    <span v-if="'<?=$this->session->userdata('user_role')?>' === '<?=User_role::FRANCHISE?>' && template.staff_name" class="d-inline orange-text">({{ template.staff_name }})</span>
                                </p>
                                <p class="fonts11">
                        ...
                        <td>{{ formatDate(template.follow_up_date) }}</td>
                        <td>{{ getStatus(template.current_pull) }}</td>
                        <td>
                            <div class="tbl-action-wrap">
                                <a v-if="'<?= $this->session->userdata('user_role')?>' === '<?=User_role::FRANCHISE?>'" :href="`<?php echo base_url(); ?>franchise/enquiry/editinquiry/${template.id}`" class="single-action btn-warning btn-sm mr-2"><img src="<?php echo base_url('assets/img/edit-2.svg'); ?>"></a>
                                <a v-if="'<?= $this->session->userdata('user_role')?>' === '<?=User_role::FRANCHISE_STAFF?>'" :href="`<?php echo base_url(); ?>franchise/enquiry/staffeditinquiry/${template.id}`" class="single-action btn-warning btn-sm mr-2"><img src="<?php echo base_url('assets/img/edit-2.svg'); ?>"></a>
                                <button v-if="'<?= $this->session->userdata('user_role')?>' === '<?=User_role::FRANCHISE?>' || '<?= $this->session->userdata('user_role')?>' === '<?=User_role::FRANCHISE_STAFF?>'" class="single-action whatsapp open_whatsup_modal" :value="template.id"><img src="<?php echo base_url('assets/img/whatsapp.svg'); ?>"></button>
                                <a class="get_follow_up" href="javascript:void(0)" :value="template.id"><img src="<?php echo base_url('assets/img/eye.svg'); ?>"></a>
                                <a class="box-btn bg-transparent add_interview_click" data-bs-target="#add_interview" data-bs-toggle="modal" href="javascript:void(0)" :value="template.id"><img src="<?php echo base_url('assets/img/add.svg'); ?>">Add deadline</a>
                                <a class="box-btn bg-transparent add_followup_click" data-bs-target="#add_followup" data-bs-toggle="modal" href="javascript:void(0)" :value="template.id">Follow Up</a>
                                <a v-if="'<?= $this->session->userdata('user_role')?>' === 'FRANCHISE' && template.passing === 'yes' && template.mobile_no !== ''" class="btn btn-success btn-sm get_visa_data" href="javascript:void(0)" :value="template.mobile_no"><i class="fa fa-cc-visa"></i></a>
                                <a v-if="'<?= $this->session->userdata('user_role')?>' === 'FRANCHISE' && template.emailpassing === 'yes' && template.email !== ''" class="btn btn-success btn-sm get_visa_data" href="javascript:void(0)" :value="template.email"><i class="fa fa-cc-visa"></i></a>
                                <button v-if="template.dtotal > 0" class="btn btn-success btn-sm get_enquirey_document" href="javascript:void(0)" :value="template.id"><i class="fa fa-file"></i></button>
                            </div>
                        </td>
                    </tr>
    ...
    watch: {
      dataFromServer(newValue, oldValue) {
         ...
         nextTick(() => {
           ...
           this.dataTable=$("#responsive-datatable-new").DataTable();
      ...
      fetchData(page,formData) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        //self.message = "Data loaded";  // Update Vue instance data
