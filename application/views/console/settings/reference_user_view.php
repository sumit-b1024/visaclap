       ...
       foreach ( $_list as $list )
       {
        ...
        <tr id="r-<?= $list->user_id; ?>">
         <td>
          ...
          <td>
           <a href="javascript:void(0)" class="btn btn-icon btn-primary btn-sm mr-2 getfranchise" data-uid="<?=$list->user_id?>" title="Franchise">
            <i class="fa fa-eye"></i>
           </a>
...
<div class="modal fade" id="franchise_modal">
  <div class="modal-dialog modal-xl" role="document">
     <div class="modal-content modal-content-demo">
        <form class="assign_country_form">
           <input type="hidden" name="user_id" id="user_id">
           <div class="modal-header">
              <h6 class="modal-title"><b>Franchise List</b></h6><button type="button" aria-label="Close" class="btn-close" data-bs-dismiss="modal"><span aria-hidden="true">&times;</span></button>
           </div>

          <div class="modal-body">
            <div class="row">
                 <div class="table-responsive reffranchise">
                 </div>
            </div>    
          </div>
...
get_all_country();
   $('#responsive-datatable').on('click', '.getfranchise', function (e) {
    ...
    $.ajax({
            type: 'POST',
            url: base_url + 'settings/get_franchise',
            data:{'reference' : userid},
            
            success: function(data) {
                $('.reffranchise').html("");
         
         $('.reffranchise').html(data);
         $('#reffranchise-datatable').DataTable();
          
          
 
            }
         });
    $('#franchise_modal').modal('toggle');  
