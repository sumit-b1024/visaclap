<div class="row">
 <div class="col-md-12 col-xl-12">
  ...
  <div class="card-body">
    ...
    <form id="hotel_form" class="form-horizontal well" method="POST" enctype="multipart/form-data">
     <div class="row">
      <div class="col-md-12 input-inside mb-1">
       <label class="form-label">Name <span class="text-red">*</span></label>
       <input type="text" name="name" id="name" placeholder="Name" class="form-control" value="<?= isset($view) ? $view->name : ""; ?>" />
       <span id="error_name" class="small text-danger"></span>
     </div>

     <?php if( isset($is_meta) && !empty($is_meta) && in_array($is_meta, [Meta::ENQUIRY_CATEGORY]) )
     { ?>

      <input type="hidden" name="enquiry_img_icon" value="<?=  isset($view) && $view->enquiry_icon_image != "" ? $view->enquiry_icon_image : ""; ?>">

      <div class="col-md-4">
       <label class="form-label">Icon Image</label>
       <input type="file" name="enquiry_category_img" id="enquiry_category_img"  class="form-control" accept="image/*" />
       <span id="error_icon_image" class="small text-danger"></span>
     </div>
 ...
 <div class="row">
  ...
  if( isset($is_meta) && !empty($is_meta) && in_array($is_meta, [Meta::HOTEL, Meta::TOURIST_ATTRACTION]) )
  {
   ...
   <div class="col-md-6 input-inside mb-1">
    <label class="form-label">Address</label>
    <textarea name="address" id="address" rows="2" placeholder="Address" class="form-control" rows="2"><?= isset($view) ? $view->address : set_value('address'); ?></textarea>
    <span id="error_address" class="small text-danger"></span>
  </div>
  <div class="col-md-3 input-inside">
    <label class="form-label">Country</label><br/>
    <select class="form-select country select2-show-search" name="country" id="country" data-placeholder="Choose country">
...
{
  ?>
  <div class="col-md-3 input-inside mb-1">
 ...
 <div class="col-md-3 input-inside mb-1">
   ...
   <select class="form-select" id="room_category" name="room_category" data-placeholder="Choose room category">
...
<div class="row">
 ...
 { ?>
  ...
  <div class="row">
   ...
   <div class="col-md-6 input-inside mb-1">
     ...
     <input type="number" min="1"  class="form-control duration" name="duration" id="duration" placeholder="Enter Duration" value="<?= isset($view) && $view->duration != "" ? $view->duration : set_value('duration'); ?>">
 ...
 <div class="row ">
   ...
   <div class="col-md-12 input-inside">
    <label class="form-label">Choose Category</label><br/>
...
<div class="row">
  <div class="col-sm-6 col-md-4 form-btns">
   <button type="submit" id="submit_btn" class="box-btn fill_primary mt-2 mb-0">
    Submit
  </button>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    if(url == base_url + "settings/add-meta/tourist_attraction" || url == base_url + "settings/edit-meta/tourist_attraction"){
     $("#name").autocomplete({
      source: function (request, response) {
        $.ajax({
          url: base_url + 'settings/get_auto_suggestion_by_name',
...
<script type="text/javascript">
  ...
  jQuery(document).on('click', '.remove_row', function() {
               ...
               var masterid =  $(this).attr('data-master');
                ...
                if (total_tr > 1 && r_id == 0) {
                  ...
                  function () {
                    $.ajax({
                      url: base_url + "settings/remove_things",
                      type : "POST",
                      data : {
                        "r_id": r_id,
                      },
                      ...
                      success : function(data){
                        ...
                        {
                          window.location.href = base_url + 'settings/edit-meta/tourist_attraction/'+masterid;
                        }else {
                          swal({
                            title: "error",
  ...
  $('#country').change(function(){
    ...
    if(countryid){
      ...
      $("#city").empty();
               ...
               // if(url == base_url + "settings/add-meta/tourist_attraction"){
               //   $("#name").autocomplete({
               //    source: base_url + 'settings/get_auto_suggestion_by_name',
       ...
       var file_nm = $("#enquiry_category_img").get(0);
       if( typeof file_nm != 'undefined' || file_nm != null ){
        for (var i = 0; i < $("#enquiry_category_img").get(0).files.length; ++i) {
         var file1=$("#enquiry_category_img").get(0).files[i].name;
         if(file1){                        
          var file_size=$("#enquiry_category_img").get(0).files[i].size;
          if(file_size < 20971520){
           var ext = file1.split('.').pop().toLowerCase();                            
           if($.inArray(ext,['jpg','jpeg','gif','png','webp','svg'])===-1){
            $('#error_icon_image').text("Invalid file extension.please select only 'jpg, jpeg', gif, png, webp, svg' this type of file.");
            return false;
          }
        }else{
         $('#error_icon_image').text("Image size is too large.upload less than 20 mb.");
         return false;
       }                        
     }
   }
 ...
 if(isValid) {
  submit_form();
        ...
        // var formData = new FormData('#hotel_form');
          ...
          $('#submit_btn').removeAttr('disabled');
                // setTimeout(function() {
                 if(data.status == 'success') {
                  ...
                  Swal.fire("info", data.message);
                } else {
                  Swal.fire("warning", "Error! Unable to complete process.");
                }
                // }, 500);
