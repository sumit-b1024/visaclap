...
class Itenerary extends MY_Controller
{
	...
	function add(){
		/** page level css & js * */
		$this->content->extra_js  = array('table-data','date-picker/date-picker','select2.full.min', 'select2','sweet-alert/sweetalert.min','sweet-alert','time-picker/jquery.timepicker','time-picker/toggles.min');
		$this->content->extra_css = array('custom');
		$title = 'Add Itinerary';
		$this->content->breadcrumb = array(
			'Dashboard'      => 	'',
	...
	function get_itinerary_tbl_records(){
		...
		$data['_view'] = $this->setting_model->get_all_itenerary($post['destination'],$post['city'],Itenerary_local_global_module::IS_ADMIN);
		$this->load->view('itenerary/itenerary_tbl_view',$data);
	...
	function store_tenerary_data(){
		...
		if($post){
			$masater_array = array(
				...
				'is_admin_or_franchise' => Itenerary_local_global_module::IS_ADMIN,
	...
	function get_titles_by_cc(){
		...
		if(isset($post) && $post['destination'] > 0 && isset($post) && $post['city'] == ""){
			$data 	= 	$this->setting_model->get_all_itenerary_destination($post['destination'],0,Itenerary_local_global_module::IS_ADMIN);
	...
	function create_itinerary_by_turist_att_admin(){
		$post = $this->input->post();
	...
	function generate_autometic_itinerary(){
		if($this->input->post()){
			...
			$get_first_location = $this->setting_model->generate_autometic_itinerary_details($this->input->post('city'),$this->input->post('destination'),Itenerary_local_global_module::IS_ADMIN);

			if(!empty($get_first_location)){

				$get_all_destinations = $this->setting_model->generate_autometic_itinerary_details_by_itenerary($this->input->post('city'),$get_first_location->latitude,$get_first_location->longitude,$get_first_location->meta_id,$this->input->post('itinerary_star'),$this->input->post('turist_category'),Itenerary_local_global_module::IS_ADMIN);
				$post = $this->input->post();
				if(!empty($get_all_destinations)){
					// $get_destination_nm = $this->setting_model->get_country_name($post['destination'],TBL_COUNTRIES_SUPPLIER);
					// $get_city_nm = $this->setting_model->get_country_name($post['city'],TBL_CITIES_SUPPLIER);
					...
					foreach ($get_all_destinations as $key => $value) {
						...
						// echo $get_distance_km.'<br>';
						$count_minutes_by_km =  round($get_distance_km) * 5 + 20 + $value->duration;
						// $count_minutes_by_km =  round($value->distance) * 5 + 20;
						// $place_minute = $count_minutes_by_km;
						$place_minute = $count_minutes_by_km;
    					// $count_minutes_by_km =  round($value->distance_in_km) * 5 + 20;
						// $count_minutes_by_km =  round($value->distance) * 5 + 20;
						// $place_minute = $count_minutes_by_km;
						// $place_minute = $value->duration + 60;
	...
	private function load_view($viewname = 'add_edit_new_enquiry', $page_title)
	{
		...
		$this->masterpage->addContentPage('itenerary/'.$viewname , 'content', $this->content);
